...
// LM head — overridable so the same kernel serves the Qwen3-TTS talker,
// whose codec head has a 3072-entry vocabulary instead of 151936.
...
__global__ void __launch_bounds__(LDG_BLOCK_SIZE, 1) ldg_decode_kernel_direct(
  ...
  // Barrier words are zeroed by the host (stream-ordered memsets in the
  // launcher) before this kernel starts. The previous in-kernel reset raced:
  // block 0's reset of barrier_sense could land after another block had
  // already published sense=1 for the entry barrier, leaving late blocks
  // spinning on sense==0 forever. With clean state guaranteed at entry, no
  // entry barrier is needed — the first grid.sync() (local_gen=0) serves as
  // the rendezvous.
  AtomicGridSync grid{barrier_counter, barrier_sense, (unsigned int)gridDim.x,
                      0};
...
extern "C" void launch_ldg_decode_direct(
  ...
  // Stream-ordered reset of all barrier/flag words. Doing this on the host
  // (instead of inside the kernel) removes the entry-barrier race between
  // block 0's reset and other blocks' arrival.
